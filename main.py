...
pygame.display.set_caption("Plane Showdown!")
...
WIDTH_BORDER = 10
BORDER = pygame.Rect((WIDTH-WIDTH_BORDER)//2, 0, WIDTH_BORDER, HEIGHT)
...
YELLOW_HIT = pygame.USEREVENT + 1 # Customs event (unique)
...
def draw_window(red, yellow, red_bullets, yellow_bullets, red_health, yellow_health):
    ...
    WIN.blit(SPACE, (0,0)) # PyGame don´t erase previous objetcs
    ...
    #3. Print surfaces into the screen
    ...
    pygame.display.update() #1. Every time you add something new

def yellow_handle_movement(keys_pressed, yellow):
    if keys_pressed[pygame.K_a] and yellow.x - VELOCITY > 0: #LEFT
        yellow.x -= VELOCITY
    if keys_pressed[pygame.K_d] and yellow.x + VELOCITY + yellow.width - 15 < BORDER.x: #RIGHT
        yellow.x += VELOCITY
...
def handle_bullets(yellow_bullets, red_bullets, yellow, red):
    for bullet in yellow_bullets:
        ...
        if red.colliderect(bullet): # Worth if both objects are Rectangule
...
def draw_winner(text):
    ...
    pygame.time.delay(5000) #Show winner text
...
def main():
    ...
    run = True
    while run:
        clock.tick(FPS) #2. Control the frame rate
        for event in pygame.event.get():    
            if event.type == pygame.QUIT:
                run = False
                pygame.quit()
        
            if event.type == pygame.KEYDOWN: # Press key and "KEYUP" -> release key
                ...
                if event.key == pygame.K_RCTRL and len(red_bullets) < MAX_BULLETS:
                    bullet = pygame.Rect(red.x - 2, red.y + red.height//2, 10, 5)
